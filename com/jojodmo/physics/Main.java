...
//	//private static boolean checksMaterials(){
//		return !doMaterials.isEmpty();
//	}
	...
	private static boolean shouldDoPhysics(Block b){
		...
		if(!hookedWG){return (world != PhysicsState.DISABLE) || matP;}
	...
	private static List<Location> ignorePhysics = new ArrayList<>();
...
//			e.setCancelled(true);
//			e.getBlock().setType(newType, false);
//
//			e.getBlockReplacedState().setType(newType);
//			e.getBlockReplacedState().update(true, false);
		}

//		else if(!shouldDoPhysics(e.getBlock().getWorld()) && !shouldDoPhysicsAround(e.getBlock())){
////			e.setCancelled(true);
////			e.getBlock().setType(newType, false);
//////			e.getBlockReplacedState().setType(newType);
////			e.getBlockReplacedState().update(true, false);
...
//	public boolean shouldDoPhysicsAround(Block b){
////		Location
////		for(int x = -1; x <= 1; x++){
////			for(int y = -1; y <= 1; y++){
////				for(int z = -1; z <= 1; z++){
...
//	}
	...
	public boolean hasPhysicsAround(Block b){
		return true;
	}

	@EventHandler
	public void blockPhysics(BlockPhysicsEvent e){
		//System.out.println("CHECKING BLOCKPHYSICS");
		// || shouldCancelPhysicsEvent(e.getSourceBlock())
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if(cmd.getName().equalsIgnoreCase("physics")){
			if(args.length == 0 || args[0].toLowerCase().matches("info|plugin|pl|ver|version")){
				...
				final String worldName = args.length > 1 ? args[1] : (sender instanceof Player ? ((Player) sender).getWorld().getName() : "null");
				World world = null;
				...
				if(args[0].toLowerCase().matches("enable|true|yes|on|1")){
					if(global){setDoPhysicsGlobal(PhysicsState.OBEY_WORLDGUARD);}
					else{setDoPhysics(world, PhysicsState.OBEY_WORLDGUARD);}
					...
					if(global){setDoPhysicsGlobal(PhysicsState.DISABLE);}
			...
			//sender.sendMessage(prefix + "Usage: /physics <enable/disable/current> [world]");
			return true;
		}
		return false;
	}
